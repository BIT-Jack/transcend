...
class InteractionDataset(Dataset):
    def __init__(self, filenames, scenarionames, stage, para, mode, filters=True):
        self.stage= stage
        self.para = para
        ...
        if stage == 'train':
            ...
            for filename in filenames:
                data = np.load(abs_dir+stage+'/'+filename+'_'+scenarionames+'.npz', allow_pickle=True)
            ...
            if self.use_sem:
                data_mask = np.load(abs_dir+'/sem_train.npz', allow_pickle=True)
                self.mask = data_mask['mask']
            else:
                data_mask = np.load(abs_dir+stage+'/mask_train_'+scenarionames+'.npz', allow_pickle=True)
                self.mask = data_mask['mask']
        else:
            data = np.load(abs_dir+stage+'/'+filenames[0]+'_'+scenarionames+'.npz', allow_pickle=True)
            ...
            if stage=='val':
                if filenames[0]=='val':
                    if self.use_sem:
                        data_mask = np.load(abs_dir+'/sem_val.npz', allow_pickle=True)
                    else:
                        data_mask = np.load(abs_dir+stage+'/mask_val_'+scenarionames+'.npz', allow_pickle=True)
                    self.Y = data['intention']
                    if mode=='lanescore':
                        self.S = data['lanescore']
                else:
                    if self.use_sem:
                        data_mask = np.load(abs_dir+'/sem_valall.npz', allow_pickle=True)
                    else:
                        data_mask = np.load(abs_dir+stage+'/mask_valall_'+scenarionames+'.npz', allow_pickle=True)
                    self.Y = data['intention']
                    if mode=='lanescore':
                        self.S = data['lanescore']
            if stage=='test':
                if self.use_sem:
                    data_mask = np.load(abs_dir+'/sem_test.npz', allow_pickle=True)
                else:
                    data_mask = np.load(abs_dir+stage+'/mask_test_'+scenarionames+'.npz', allow_pickle=True)
            self.mask = data_mask['mask']
